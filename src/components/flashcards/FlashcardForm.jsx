...
export const FlashcardForm = ({ flashcard, onSubmit, onCancel, loading }) => {
  const { disciplines } = useDisciplines()
  const { contents, getContentsByDiscipline } = useContents()
  ...
  useEffect(() => {
    if (flashcard) {
      ...
      if (flashcard.content?.discipline_id) {
        setSelectedDiscipline(flashcard.content.discipline_id)
      }
    }
  }, [flashcard])
  ...
  const handleSubmit = async (e) => {
    ...
    if (uploadedFiles.length > 0) {
      const hasImages = uploadedFiles.some(file => file.mime_type.startsWith('image/'))
      const hasPdfs = uploadedFiles.some(file => file.mime_type === 'application/pdf')
    ...
    await onSubmit(flashcardData, uploadedFiles)
  }
